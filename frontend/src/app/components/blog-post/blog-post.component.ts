...
export class BlogPostComponent implements OnInit, OnDestroy {
  ...
  formatContent(content: string): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const isOrderedBullet   = /^\d+\.\s/.test(line);
      const isUnorderedBullet = /^[-•*→►▸✓✔·]\s/.test(line);
      const hasEmojiStart     = /^[\u{1F300}-\u{1FFFF}\u{2600}-\u{26FF}✨💡🎯🧠📌🔑🌟⭐🏆🔍📝📊📈📉📁✦✧✏️]/u.test(line);
      const endsWithColon     = /^[A-Z].{3,50}:$/.test(line);
      const isAllCaps         = /^[A-Z][A-Z\s\d\-:&]{5,}$/.test(line) && line.length < 70;

      if (hasEmojiStart || isAllCaps) {
        if (inList) { result.push(`</${listType}>`); inList = false; }
        const tag = result.filter(r => r.startsWith('<h')).length === 0 ? 'h2' : 'h3';
        result.push(`<${tag}>${this.escapeHtml(line)}</${tag}>`);
      } else if (endsWithColon) {
        if (inList) { result.push(`</${listType}>`); inList = false; }
        result.push(`<h4>${this.escapeHtml(line)}</h4>`);
      } else if (isUnorderedBullet) {
        if (!inList || listType !== 'ul') {
          if (inList) result.push(`</${listType}>`);
          result.push('<ul>'); inList = true; listType = 'ul';
        }
        result.push(`<li>${this.escapeHtml(line.replace(/^[-•*→►▸✓✔·]\s+/, ''))}</li>`);
      } else if (isOrderedBullet) {
  ...
  shareOn(platform: string): void {
    ...
    const title = encodeURIComponent(this.post?.title || 'Check this out on InkDrop');
